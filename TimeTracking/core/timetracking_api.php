...
* @access public
*/
function plugin_TimeTracking_stats_get_project_array( $p_project_id, $p_from, $p_to, $p_user_id, $p_bug_id = ' ') {
	...
	$t_query = '
	SELECT * FROM 
	(SELECT tr.id id, u.id user_id, u.realname username, b.project_id project_id, p.name as project_name, bug_id, expenditure_date, hours, timestamp, category, info, true AS is_new_tt 
	...
	UNION
	SELECT bn.id id, u.id user_id, u.realname username, b.project_id project_id, p.name as project_name, bn.bug_id bug_id, DATE_FORMAT(FROM_UNIXTIME(bn.date_submitted), \'%Y-%m-%d\') as expenditure_date, 
	...
	if( !is_blank($p_user_id) ) {
		$t_query .= " AND user_id = " . db_param();
		$t_query_parameters[] = $p_user_id;
	}
